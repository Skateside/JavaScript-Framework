...
define([
     ...
     *
     *      util.String.match("a<b>c<d>e", /<(\w+)>/, function (matches) {
     *          // matches = ["<b>", "b"]
     *          // matches = ["<d>", "d"]
    ...
    function tokenise(string, pattern, handler, context) {
        ...
        var str = interpret(string);
        var reg = core.removeFlags(pattern, "g");
        var parts = [];
        var match;
        ...
        while (str.length) {
            ...
            if (!match || match[0].length === 0) {
                ...
                parts.push(
                    ...
                    interpret(handler.call(context, match))
                );
    ...
    function supplant(string, replacements, pattern) {
        ...
        return tokenise(string, pattern, function (matches) {

            var prefix = matches[1];
            var whole = matches[2];
            var value = core.objectAccess(replacements, matches[3]);
            var replacement = isStringy(value)
                ? value
                : whole;
    ...
    core.assign(string, {
         ...
         *  - context (?): Optional context for the `handler`.
         *
         *  An alias of [[util.String.tokenise]] to aid developers used to
         *  American English.
         **/
        tokenize: tokenise
