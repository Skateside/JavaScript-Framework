(function () {
    ...
    var $a = {},
         ...
         * object that cannot easily be converted into an `Array` will return an
         * empty `Array` but it appears to break when a `Function` is passed to
         * be converted. This is because in JavaScript, `Function`s have a
     ...
     *
     * Helper function for adding multiple methods to a class's `prototype`. See
     * [[Class.addMethod]].
     **/
     ...
     * property. This property will call the parent's method and allow arguments
     ...
     * the function once. This function is very useful for times when a function
     * may be called too frequently and only the end result is required.
     *
     ...
     * its context. This function is handy for iterating over array-like objects
     * as well as arrays, something that `Array.prototype.every` can handle, but
     * the array-like objects rarely have a "every" method.
     **/
     ...
     * its context. This function is handy for iterating over array-like objects
     * as well as arrays, something that `Array.prototype.filter` can handle,
     * but the array-like objects rarely have a "filter" method.
     **/
     ...
     * find the first one that satisfies the result. The `filter` will take each
     ...
     * - array (?): Object to test.
     *
     * Checks to see if an object is array-like, i.e. it could be converted into
     * an Array using [[$a.from]]. Think of this function as a companion
     ...
     *
     * Checks to see if the given object is numeric. This does not mean that the
     * object is a number, but rather that the object could be used as a number.
     *
     *      $n.isNumeric(1); // -> true
     *      $n.isNumeric('1'); // -> true
     ...
     *
     * Makes a function that will check a given object has the properties passed
     ...
     * its context. This function is handy for iterating over array-like objects
     * as well as arrays, something that `Array.prototype.map` can handle, but
     * the array-like objects rarely have a "map" method.
     **/
     ...
     * 
     * Allows for function currying. Each argument after the first one is passed
     * to the function (first argument). Any instances of `undefined` will be
     * replaced with arguments passed to the function at run time. Additional
     * arguments passed to the function will still be passed.
     *
     *      function add(a, b) {
     *          return a + b;
     *      }
     ...
     *
     * If a `value` is provided, an `Object` is returned rather than an `Array`,
     * the `Object` is a key/value set based on the entries in the array, with
     * the `Object`'s keys being the `property` and the values being `value.
     *
     *      var arr = [
     *          {name: 'foo', value: 1},
     *          {name: 'bar', value: 2},
     ...
     *
     * Removes items from an array, if they are found. The original array is not
     * affected.
     *
     ...
     * its context. This function is handy for iterating over array-like objects
     * as well as arrays, something that `Array.prototype.some` can handle, but
     * the array-like objects rarely have a "some" method.
     **/
     ...
     * heavy than constructing a class and evaluating the string, especially for
     ...
     * pattern should define 3 groups: the character before the placeholder, the
     * whole placeholder and the key within the placeholder. For example, here
     * is a replacement using two braces.
     *
     *      var string = 'Testing {{framework}}';
     *      var map = {framework: 'Song'};
     *      var pattern = /(^|.|\r|\n)(\{\{(.*?)\}\})/;
     ...
     * Templates](http://api.prototypejs.org/language/Template/), it is based on
     * an idea by [James
     * Padolsey](http://james.padolsey.com/javascript/straight-up-interpolation/).
     * 
     * An easier-to-use wrapper exists in the form of [[$s.supplant]].
     **/
    ...
    augment(Template, {
         ...
         * This cache helps reduce additional work for [[$s.Template]] instances
         * that were not saved in a variable or created using [[$s.supplant]].
         * It should be treated as **private**, but if a future extension
         * overrides [[$s.Template#createFunc]], it may be useful to be able to
         * empty this cache.
         **/
        cache: {},
